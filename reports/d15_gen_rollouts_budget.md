...
- **冒烟实测回填(2026-08-20,job 176710098,walltime 18m28s ≈ 22 SU)**:
  E_paired = **5.0**(bs8 2.3 min vs bs1 11.5 min,同 64 行),门 E≥1.5 过;
  R_bs1_smoke = 0.0928 row/s 与 pilot R₁ = 0.0958 差 ~3%,交叉验证锚点;
  全量外推 **21.5 GPU·h ≈ 1,548 SU**(落预算区间 [1,290, 2,581] 内)。
  断点演练:110s SIGKILL 后 40 行存活,续跑补 24 行,重扫零 dirty、
  二次 repair 字节不变;且 drill 与 bs8 不间断跑 64/64 token 全同
  (续跑点 40 为 8 的整倍,批组成一致→确定性复现的旁证)。
  对照标定(非门,预登记类别):bs8 vs bs1 逐行 token 全同 28/64,
  eos_hit 两臂同为 41/64,均长 385.7 vs 376.4——批内 fp16 扰动量级登记。
- **预算数字**:
