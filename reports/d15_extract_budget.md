...
- 【收账后回填】精确数:队列① 报告出全池 L̄(分源)后,按
  ...
  超出重报
- **回填(2026-08-21,d15_gen_rollouts_report §5)**:L̄=386.7(全池
  实测,Σ L=14,337,784)→ T̄=837.2 → **1,113 GB ≈ 1.04 TiB**,对
  预估 +1.8%,±5% 内**预算不改**;算力同步回算 4.49 GPU·h ≈ 323 SU
  (预期 330 内,上限 516 不动)
...
## 4b. 体积判据修订(2026-08-21,用户裁定入档)

- pilot 判据 2(朴素 ×72.42 外推 ±10%)越界 +19.9%,归因成立:池首
  512 行全 DOCCI、几乎顶像素 cap(P̄=640.2),行集构成不代表全池——
  与冒烟 E 口径同类坑,两处各修一半的教训归档(E 修了测量口径,
  体积判据当时漏修)
- **判据改分层口径,双数字入档**:分层预测 **1,119 GB**(非 DOCCI
  P̄≈390.6 解出)/ DOCCI 顶格上界 **1,335 GB**;收官报实测对两数
  落点,作为归因模型的免费检验
- 详见 reports/d15_extract_pilot_report.md
