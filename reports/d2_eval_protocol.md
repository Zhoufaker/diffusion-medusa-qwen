...
## 门
**G0 复跑保真**:
- 臂② 在 old-100 上通过字节级回归门(冠军被忠实复跑的证明)
- 读 ARCHIVE_POLICY_V2_ANCHOR.json 确认 v2 计时硬件:
  - 若与 A100 同类:臂② 复跑加速比须落 1.732× ± 2σ_batch,
    σ_batch = v2_rebaseline 日志中等价配置的批间标准差
    (数值:**σ_batch = 0.0537,2σ = 0.1074,带 [1.6246, 1.8394]**。
    ...
    1.7741, 1.8227, 1.7278, 1.7552, 1.6945, 1.7149]。锁定时填入);
    超差先查计时环境
  - 若原为 V100:登记"硬件迁移、无数值对照门",A100 复跑值即新基准
  - **锁定时判定(写死)**:ARCHIVE_POLICY_V2_ANCHOR.json 无硬件字段;
    实查 v2 计时作业 175598529 与 175680071/72 均为 `#PBS -q gpuvolta`
    (12 核/96G 单 V100 形态,PBS 记账佐证)→ **适用"原为 V100"分支:
    登记硬件迁移、无数值对照门,A100 复跑值即新基准**。
    σ_batch 数值仅作登记保留,不构成门。
- 附登记(非门):old-100 字节门的参照 archive
  (v2_rebaseline/dyn_k8_n24.spec_archive.json)录制于 V100;A100 复跑存在
  已量化的 ~0.16%/位置硬件翻转(四臂归因矩阵),字节门结果按实报告,
  失配样本逐条列出交导师裁决,不自动判负
